...
Verify without a real cron, using a saved fixture:
    ...
    # First run against an empty cache seeds state only (no alert — there's no
    # prior "booked" observation to transition from). Run it again after the
    # site changes to see an alert fire.
...
If the site redesigns, re-probe it (push a throwaway workflow to GitHub
Actions, since this sandbox can't reach the domain directly) and update
parse_slots() accordingly.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    for key, slot in tracked.items():
        ...
        # Only alert on an observed booked->free transition. A slot seen for
        # the first time (no prior cache entry, e.g. cold start or cache
        # loss) has no known "before" state, so it's recorded but not
        # alerted on -- mirrors tennis_app's cache-loss behaviour of staying
        # silent rather than firing a spurious alert on every fresh cache.
        if prev_slot is not None and not prev_slot["free"] and slot["free"]:
