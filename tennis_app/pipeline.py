...
def run(
    ...
    *,
    ...
    Execute the full pipeline:
      ...
      3. Find Wednesday-evening (>=19:00) slots that flipped from booked to free
    ...
    logging.info("Computing Wednesday-evening (>=19:00) openings…")
    ...
    if opened_keys:
        ...
        if opened_indices:
            ...
            if notify:
                logging.info("Sending email: %d Wednesday-evening slot(s) opened up…", len(opened_keys))
                send_email("Tennis court free — Wednesday evening!", opened_df)
            else:
                logging.info(
        ...
        logging.info("No Wednesday-evening (>=19:00) openings; no email.")
